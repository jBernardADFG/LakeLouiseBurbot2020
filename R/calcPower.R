...
#' @return Returns the power of concluding that the between transect variance has increased since 2005. The rows of the matrix coorespond to the entries in mu_vec while the columns coorespond to entries in sig_vec.
#' @examples 
...
#' mu_vec <- c(0.4,0.5, 0.6)
#' sig_vec <- c(0.05, 0.35, 0.8, 1.15)
#' alpha <- 0.10
#' calc.power(ct_mat, ts_mat, mu_vec, sig_vec, alpha,10)
#' @export

calc.power <- function(ct_mat, ts_mat, mu_vec, sig_vec, alpha, n_mc=100){
